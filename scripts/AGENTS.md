...
- `upstream-sync` owns upstream synchronization: status, plan, isolated-worktree replay (`prepare`/`resume`), the compatibility gate (`verify`), base promotion (`promote`), and fork-surface measurement. It reads `maint/upstream.toml`, writes evidence to `maint/reports/`, and must stay deterministic in clean checkouts.
